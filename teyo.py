...
def main():
    ...
    session_id = str(Write.Input("Nhập Session ID: ", Colors.cyan_to_blue, interval=0))

    last_username = get_profile(session_id, device_id, iid)
    if last_username != "None":
        print(f"Tên id của bạn là: {last_username}")
        new_username = input("Nhập id mới: ")
        print(change_username(session_id, device_id, iid,last_username, new_username))

    else:
        print("Lỗi session ID hoặc lỗi khác")

if __name__ == "__main__":
    main()
k = input("Nhấn enter để thoát")
#os.system("python teyo.py")